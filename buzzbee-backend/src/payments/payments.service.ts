...
export class PaymentsService {
  ...
  async verifySucceededSponsorshipIntent(params: {
    ...
    if (!SPONSORSHIP_PLANS[days]) {
      throw new BadRequestException('Invalid sponsorship duration');
    }

    const boostedEvent = await this.autoBoostEvent(params.eventId, days);

    return {
      success: true,
      days,
      eventId: boostedEvent.id,
      sponsoredUntil: boostedEvent.sponsoredUntil,
    };
  }
