...
export default function EventDetailsPage() {
  ...
  const isBoostEligible = !event.isSponsored && event.sponsorshipStatus !== 'PENDING' && event.status === 'APPROVED';
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:w-2/3 p-8 flex flex-col justify-between">
            ...
            <div className="flex gap-4 border-t border-gray-100 pt-6">
              <Link href={`/organizer/create-event?id=${event.id}`}>
                <Button variant="outline" className="flex-1 w-full bg-white text-gray-700">Edit Event</Button>
              </Link>
              
              {isBoostEligible && (
                <Button 
                  className="flex-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:from-amber-600 hover:to-orange-600 border-none shadow-lg transition-transform transform hover:scale-105"
                  icon={<Zap size={18} />}
              ...
              {event.sponsorshipStatus === 'PENDING' && (
                <Button 
                  className="flex-1 bg-amber-100 text-amber-800 border-none cursor-default"
                  disabled
                >
                  Boost Request Pending
                </Button>
              )}
              {event.status !== 'APPROVED' && !isBoostEligible && event.sponsorshipStatus !== 'PENDING' && !event.isSponsored && (
                <div title="Event must be approved to be boosted" className="flex-1">
                  <Button 
                    className="w-full bg-gray-100 text-gray-500 border-none cursor-default tooltip"
                    disabled
                  >
                    Boost Unavailable
                  </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{event.attendees || 0} / {event.capacity || "Unlimited"}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{Number(event.price) > 0 ? `Rs. ${event.price}` : "Free"}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center">
              ...
              <p className="text-2xl font-bold text-gray-900">Rs. {event.revenue || 0}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col items-center justify-center">
              ...
              <p className={`text-xl font-bold ${event.status === 'APPROVED' ? 'text-green-600' : event.status === 'PENDING' ? 'text-yellow-600' : 'text-red-600'}`}>
                {event.status}
              </p>
            </div>
        </div>
      ...
      <Modal isOpen={boostModalOpen} onClose={() => !boosting && setBoostModalOpen(false)} title="Boost Your Event">
        {boostSuccess ? (
          ...
          <div className="py-2">
            ...
            <ul className="space-y-3 text-sm text-gray-600 mb-8 list-disc pl-5">
              ...
              <li><strong className="text-gray-900">The admin will contact you</strong> regarding approval and the associated fee processing.</li>
              <li>Once confirmed and processed, your event will be featured prominently on the attendee homepage.</li>
            </ul>
