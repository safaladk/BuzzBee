                ...
                <div className="flex items-center justify-between">
                  ...
                  {deleteModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center">
                      <div className="absolute inset-0 bg-black opacity-40" onClick={closeDeleteModal}></div>
                      <div className="bg-white rounded-lg p-6 z-10 w-full max-w-md">
                        <h3 className="text-lg font-bold mb-2">Delete event?</h3>
                        <p className="text-sm text-gray-600 mb-4">Are you sure you want to delete this event? This action cannot be undone.</p>
                        <div className="flex justify-end gap-3">
                          <button onClick={closeDeleteModal} className="px-4 py-2 rounded-lg bg-gray-100">Cancel</button>
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              {events.length === 0 ? (
                <div className="px-6 py-12 text-center">
                  ...
                  <Link href="/organizer/create-event">
                    <Button variant="primary" icon={<Plus size={20} />} onClick={() => {}}>
                      ...
                      {events.map((event) => (
                        <tr key={event.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-900">
                            {(event.price || 0) > 0 ? `Rs. ${(event.price || 0).toLocaleString()}` : 'Free'}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-900">{event.attendees || 0}</td>
                          <td className="px-6 py-4 text-sm text-gray-900">
                            {(event.revenue || 0) > 0 ? `Rs. ${(event.revenue || 0).toLocaleString()}` : 'Free'}
                          </td>
                          <td className="px-6 py-4 text-sm">
                            ...
                            <td className="px-6 py-4 text-sm">
                              <div className="flex gap-2">
                                ...
                                <button onClick={() => openDeleteModal(String(event.id))} className="p-2 text-gray-600 hover:bg-red-100 hover:text-red-600 rounded-lg transition" title="Delete">
                                  <Trash2 size={18} />
                                </button>
                              </div>
                            </td>
